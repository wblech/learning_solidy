...
const CHAIRPERSON_ROLE = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("CHAIRPERSON_ROLE"));
...
describe("Ballot", function () {
    ...
    // it("Should contract creator have role DEFAULT_ADMIN_ROLE", async function () {
    //     expect(await contract.hasRole(ethers.utils.formatBytes32String("0"), accounts[0].getAddress())).to.equal(true);
    // })

    it("Should contract creator have role CHAIRPERSON", async function () {
        expect(await contract.hasRole(CHAIRPERSON_ROLE, accounts[0].getAddress())).to.equal(true);
    })

    it("Should be possible to grant CHAIRPERSON_ROLE to another addres", async function() {
        let newChairPerson = accounts[1].getAddress();

        await contract.connect(accounts[0]).grantRole(CHAIRPERSON_ROLE, newChairPerson);
        expect(await contract.hasRole(CHAIRPERSON_ROLE, newChairPerson)).to.equal(true);
    })

    it("Should mint token to the owner", function () {
        let teste = await contract.
        console.log()
    })
